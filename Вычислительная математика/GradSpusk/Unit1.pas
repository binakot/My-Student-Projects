...
begin
  a1:=a;
  b1:=b;
  t:=(-1+sqrt(5))/2;  //находим t дл€ золотого сечени€
  x1:=b1-t*(b1-a1);   //находим х1, х2
  x2:=a1+t*(b1-a1);   //по формулам
  y1:=GetH(x,y,x1);   //находим значение функции Ќ
  y2:=GetH(x,y,x2);   //в этих точках
    repeat
      if y1>y2 then
        begin
          a1:=x1;      //дл€ дальнейшего делени€
          x1:=x2;     //оставл€ем (х1,b)
          y1:=y2;
...
begin
  ...
  i:=0;
  repeat                 //помещаем все €мы в массив
     y1:= GetH(x,y,a1);
     y2:= GetH(x,y,a1+h1);
     y3:= GetH(x,y,a1+2*h1);
...
begin
  ...
  RichEdit1.Lines.Add('Ќачальное приближение: x0 = '+floattostr(x0)+', y0 = '+floattostr(y0));
  ...
  RichEdit1.Lines.Add('_____________ ћинимум ‘ункции ____________');
